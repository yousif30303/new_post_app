        ...
        <div class="flex justify-center">
            <div class="w-6/12 bg-white p-6 rounded-lg">
                <form action="{{route('posts')}}" method="post">
                    ...
                    <div>
                        <button type="submit" class="bg-blue-500 text-white px-4 py-3 rounded font-medium mb-4">Post</button>
                    </div>
                </form>
                @if ($posts->count())
                    @foreach ($posts as $post)    
                    <div class="mb-4">
                        <a href="{{route('user.posts',$post->user)}}" class="font-bold">{{$post->user->name}}</a><span class="text-gray-600 text-sm">
